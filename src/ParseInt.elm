module ParseInt exposing
    ( parseInt, parseIntOct, parseIntHex, parseIntRadix, toRadix, unsafelyConvertToRadix, toOct, toHex, intFromChar, charFromInt
    , Error(..)
    )
...
@docs parseInt, parseIntOct, parseIntHex, parseIntRadix, toRadix, unsafelyConvertToRadix, toOct, toHex, intFromChar, charFromInt
...
toRadix radix i =
    if 2 <= radix && radix <= 36 then
        if i < 0 then
            Ok <| "-" ++ unsafelyConvertToRadix radix -i
...
-}
unsafelyConvertToRadix : Int -> Int -> String
unsafelyConvertToRadix radix i =
    if i < radix then
        String.fromChar <| charFromInt i

    else
        unsafelyConvertToRadix radix (i // radix)
            ++ (String.fromChar <| charFromInt (i |> remainderBy radix))
